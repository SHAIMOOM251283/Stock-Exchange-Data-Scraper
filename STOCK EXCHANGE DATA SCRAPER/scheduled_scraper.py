...
class Scrap:
    ...
    def convert_list_of_dictionaries_to_dataframe(self):
        self.df = pd.DataFrame(self.all_data)

    def generate_filename_with_current_date(self):
        self.filename = f"extracted_data_{datetime.now().strftime('%Y-%m-%d')}.csv"

    def save_dataframe_to_CSV_file(self):
    ...
    def run(self):
        ...
        self.convert_list_of_dictionaries_to_dataframe()
        self.generate_filename_with_current_date()
        self.save_dataframe_to_CSV_file()
    
    def schedule_the_job(self):
        schedule.every().day.at("19:44").do(self.run)
        while True:
